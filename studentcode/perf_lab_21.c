...
#ifdef MANUAL_TEST // run the following code is MANUAL TEST is Defined above *uncommented
	int howManyToCalculate = 0; // declared and zero
	tempValue = 0;
...
#define BUFF_SIZE 24
	...
	for (i = 0; i < BUFF_SIZE; i++)
	{
		...
		numberOfTestsRun++;
        // printf("Returned => %lu : Expected => %lu\n", tempValue, returnValues[i]);
...
unsigned long shadow_sequence_number(int placeNumber)
{
    if (placeNumber > MAXIMUM_SEQ_NUM || placeNumber < 1)
    {
        return ERROR_VALUE;
    }
